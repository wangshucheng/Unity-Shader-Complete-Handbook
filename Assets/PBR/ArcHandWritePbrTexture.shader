    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
				...
				//o.n_uv = TRANSFORM_TEX(v.uv, _NormalMap);
            ...
            half4 frag (v2f i) : SV_Target
            {
				...
				//float3 _NormalMap_var = UnpackNormal(tex2D(_NormalMap, i.uv));
				float3 _NormalMap_var = UnpackScaleNormal(tex2D(_NormalMap, i.uv), _NormalScale);
				float3 normalLocal = _NormalMap_var.rgb;
				float3 normalDirection = normalize(mul(normalLocal, tangentTransform)); // Perturbed normals

				i.normal = normalDirection;
