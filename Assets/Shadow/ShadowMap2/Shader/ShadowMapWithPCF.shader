Shader "Shadow/ShadowMapWithPCF" {
	Properties
	{
		//主纹理
...
#pragma vertex vert
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				//将顶点从世界坐标系转换到光空间坐标系
				half4 lightSpacePos = mul(_LightSpaceMatrix, i.worldPos);
				...
				/*
				//获得深度贴图的颜色值
				half4 depthRGBA = tex2D(_DepthTexture, pos.xy);
				...
				*/
				...
				for (int x = -1; x <= 1; x++) {
					for (int y = -1; y <= 1; y++) {
						...
						half4 pcfDepthRGBA = tex2D(_DepthTexture, samplePos);
